...
export default function NoteList({ courseId, subjectId, teacherName, subject, initialNotes }) {
    const [notes, setNotes] = useState(initialNotes || []);
    const [downloadingId, setDownloadingId] = useState(null);

    const handleDownload = async (noteId, pdfUrl) => {
        try {
            ...
            <main className="max-w-5xl mx-auto">
                ...
                <header className="mb-12 bg-white/60 backdrop-blur-md p-8 rounded-3xl shadow-sm border border-white/50">
                    <h1 className="text-3xl font-bold text-gray-900 mb-2 flex items-center gap-3">
                        ...
                        Notes by {teacherName}
                    </h1>
                    <p className="text-gray-500 font-medium">
                        {subject.courseId?.name} • {subject.name}
                    </p>
                </header>
                ...
                {notes.length === 0 ? (
                    <div className="text-center py-20 bg-white/50 backdrop-blur-sm rounded-3xl border border-white/40 shadow-xl">
                        ...
                        <p className="text-gray-400 mt-2">This teacher hasn't uploaded any notes yet.</p>
                    </div>
                ) : (
                        ...
                        {notes.map((note) => (
                            <div key={note._id} className="bg-white p-6 rounded-2xl shadow-sm hover:shadow-lg border border-gray-100 transition-all duration-300 flex flex-col justify-between h-full group">
                                <div>
                                    ...
                                    <p className="text-sm font-medium text-gray-500 mb-6">By {note.teacherName}</p>
                                </div>

                                <button
                                    onClick={() => handleDownload(note._id, note.pdfUrl)}
